...
def test_url_shortener_shorten_and_persist(
    fake_short_url_queue, fake_url_table, fake_cache, sample_url
):
    shortener = UrlShortener(
        queue=fake_short_url_queue, table=fake_url_table, cache=fake_cache
    )
    ...
    assert str(excinfo.value) == "There are no keys left on queue available_urls"


def test_url_shortener_get_long_url(
    fake_short_url_queue, fake_url_table, fake_cache, sample_url
):
    shortener = UrlShortener(
        queue=fake_short_url_queue, table=fake_url_table, cache=fake_cache
    ...
    assert str(excinfo.value) == "Short url 'unexistent' does not exist on url table"


def test_url_shortener_persist_on_table(
    fake_short_url_queue, fake_url_table, fake_cache, sample_url
):
    shortener = UrlShortener(
        queue=fake_short_url_queue, table=fake_url_table, cache=fake_cache
...
def test_url_shortener_fetch_new_short_url(
    fake_short_url_queue, fake_url_table, fake_cache, sample_url
):
    shortener = UrlShortener(
        queue=fake_short_url_queue, table=fake_url_table, cache=fake_cache
...
def test_url_shortener_add_on_cache(
    fake_short_url_queue, fake_url_table, fake_cache, sample_url, fake_redis_client
):
    shortener = UrlShortener(
        queue=fake_short_url_queue, table=fake_url_table, cache=fake_cache
    )
    ...
    assert (
        fake_redis_client.get(sample_url.short_url).decode("utf-8")
        == sample_url.long_url
    )
